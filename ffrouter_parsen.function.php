        ...
        for( $j=0; $j<$installation_count; $j++ ) {
            if($variante[$entwicklung[$i]][$installation[$j]] == 1) {
                ...
                for( $x=0; $x<$file_count; $x++) {
                    ...
                    if(isset($router_tmp[$x]['hersteller']) != true) {
                        $error_text[$err]="Unbekannten Hersteller im Dateinamen gefunden, bitte Script updaten! Bitte schreibe eine Mail an leo@freifunk-siegburg.de! (".$files[$entwicklung[$i]][$installation[$j]][$x].")";
                        $err++;
                    } else {
                        $router_neu = 1;
                        $router_count = count($router);
